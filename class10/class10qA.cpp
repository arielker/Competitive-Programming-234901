...
// time: O(ElogV).
int Kruskal(vector<iii> &edges, int n, vector<pair<int, int>> &tuples) {
...
int main() {
    ...
    vector<iii> villages_w;
    for (long long i = 0; i < n * n; i++) {
        long long w = 0;
        scanf("%llu", &w);
        iii candidate_edge = {w, {i / n, i % n}};
        if (candidate_edge.second.first < candidate_edge.second.second) {
            villages_w.push_back(candidate_edge);
        }
    }
    vector<pair<int, int>> tree_included;
    Kruskal(villages_w, n, tree_included);

    for (long long i = 0; i < tree_included.size(); i++) {
        int vil_1 = tree_included[i].first + VILLAGE_GAP;
        int vil_2 = tree_included[i].second + VILLAGE_GAP;
        printf("%d %d\n", vil_1, vil_2);
    }
