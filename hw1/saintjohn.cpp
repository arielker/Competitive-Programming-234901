...
bool segParallel(point a, point b, point c, point d) {
    return abs(cross(a - b, c - d)) < EPS;
}

bool pointInside(const point &p, const vector<point> &points) {
    auto x = points.size();
    bool ret = false;
    for (int i = 0; i < x; ++i) {
        auto j = (i + 1) % x;
...
bool pointOnPolygon(const point &p, const vector<point> &points) {
    auto x = points.size();
    for (int i = 0; i < x; ++i) {
        if (pointOnSegment(p, points[i], points[(i + 1) % x])) {
            return true;
        }
    }
    return pointInside(p, points);
}
...
int main() {
    int l, s;
    cin >> l;
    vector<point> big_lanterns;
    for (int i = 0; i < l; ++i) {
        double x, y;
        cin >> x >> y;
        big_lanterns.emplace_back(point(x, y));
    }
    sort(big_lanterns.begin(), big_lanterns.end());
    cin >> s;
    int count = 0;
    for (int i = 0; i < s; ++i) {
        double x, y;
        ...
        point p = point(x, y);
        bool check = false;
        for (int j = 0; j < big_lanterns.size(); ++j) {
            for (int k = j + 1; k < big_lanterns.size(); ++k) {
                for (int m = k + 1; m < big_lanterns.size(); ++m) {
                    vector<point> points;
