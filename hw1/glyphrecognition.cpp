...
double calc(vector<pdd> vector1, int k);
...
double calculate(vector<pdd> &points, int k) {
    int c = 0;
    double omega = PI / k;
    double theta = 2 * omega;
    double angle = theta;
    double maxA = 0.0, minA = numeric_limits<double>::max();
    for (int i = 0; i < k; ++i) {
        while (c < points.size() && points[c].first <= angle) {
            double beta = abs(points[c].first - angle + omega);
            double a = points[c].second * cos(beta);
            maxA = max(maxA, a);
            minA = min(minA, a);
            c++;
        }
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        int x, y;
        cin >> x >> y;
        double r = dist(point(x, y), point(0, 0));
        double alpha = atan2(y, x);
        points.emplace_back((alpha < 0 ? 2 * PI + alpha : alpha), r);
    }

    sort(points.begin(), points.end(),
         [](pdd a, pdd b) -> bool { return a.first < b.first; });

    double best_score = 0.0;
    int best_k = 3;
    for (int k = 3; k < 9; ++k) {
        double score = calculate(points, k);
        if (score > best_score) {
            best_k = k;
            best_score = score;
        }
    }
    cout << best_k << " " << best_score << endl;
    return 0;
}
