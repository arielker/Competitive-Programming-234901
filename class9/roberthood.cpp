...
int main() {
    ...
    for (int i = 0; i < c; ++i) {
        int x, y;
        cin >> x >> y;
        arrows.emplace_back(point(x, y));
    }
    vector<point> convex = convex_hull(arrows);
    double max_dist = dist(convex[0], convex[1]);
    for (int i = 0; i < convex.size(); ++i) {
        for (int j = i + 1; j < convex.size(); ++j) {
            max_dist = max(max_dist, dist(convex[i], convex[j]));
        }
    }
    cout << setprecision(10) << max_dist << endl;
    return 0;
}
